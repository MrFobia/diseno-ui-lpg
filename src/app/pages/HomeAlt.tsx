...
import { FONT, B3, GREEN, W, W40, W08, W04 } from "../tokens";
import { Glow } from "../components/shared/Glow";

// ── Paleta oficial de marca (branding PDF) — sin los navys custom del diseño 1 ──
const B0 = "#000000"; // Deep Black
const B1 = "#000000"; // Deep Black
const B4 = B3;         // LPG Blue (una sola variante de azul, sin el azul claro extra)
const INK  = "#0A0A0A"; // Deep Black para titulares — reemplaza el azul como color de texto por defecto
const GRAY = "#333333"; // Dark Gray oficial — labels y texto secundario

// ── Grises con contraste AA verificado (WCAG 2.1) — reemplazan los rgba() translúcidos originales ──
const DIM_DARK   = "rgba(255,255,255,0.62)"; // texto secundario sobre fondo negro — 7.4:1
const DIM_LIGHT  = "#595959";                 // texto secundario sobre fondo blanco — 7.0:1
const BODY_LIGHT = "#4D4D4D";                 // párrafos sobre fondo blanco — 8.5:1
const FAINT_LIGHT = "rgba(0,0,0,0.45)";       // iconos/decorativos sobre blanco — 3.4:1 (min. UI 3:1)
...
export default function HomeAlt() {
  ...
  const logos = [
    ...
    { src: logoCorfi,      name: "Corficolombiana" },
    { src: logoCajaSocial, name: "Banco Caja Social" },
    { src: logoUan,        name: "Universidad Antonio Nariño" },
    { src: logoGrupoAval,  name: "Grupo Aval" },
    { src: logoCcCali,     name: "Cámara de Comercio de Cali" },
    { src: logoCamacol,    name: "Camacol" },
    { src: logoCarvajal,   name: "Carvajal" },
    { src: logoColmena,    name: "Colmena Seguros" },
    { src: logoEstelar,    name: "Estelar" },
  ];
    ...
    <>
      ...
      <section className="lpg-pad" style={{ minHeight: "82vh", position: "relative", overflow: "hidden", background: B1, display: "flex", flexDirection: "column", justifyContent: "flex-end", paddingLeft: 48, paddingRight: 48, paddingBottom: 64 }}>
        ...
        <div style={{ maxWidth: 1440, width: "100%", margin: "0 auto", paddingTop: 120, position: "relative", zIndex: 1 }}>
          ...
          <Link to="/" style={{ display: "inline-block", alignSelf: "flex-start", marginTop: 28, fontFamily: FONT, fontWeight: 500, fontSize: 12, letterSpacing: "0.08em", textTransform: "uppercase", color: W, textDecoration: "none", borderBottom: `1px solid ${W}55`, paddingBottom: 3, transition: "color 0.2s, border-color 0.2s" }}
      ...
      <section style={{ backgroundColor: "#FFFFFF", borderBottom: "1px solid rgba(0,0,0,0.07)", overflow: "hidden", padding: "44px 0", position: "relative" }}>
        <div className="lpg-pad" style={{ maxWidth: 1440, margin: "0 auto", paddingLeft: 48, paddingRight: 48, marginBottom: 28, display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <span style={{ fontFamily: FONT, fontWeight: 500, fontSize: 11, letterSpacing: "0.22em", textTransform: "uppercase", color: DIM_LIGHT }}>Confían en nosotros</span>
        </div>
        <div style={{ display: "flex", overflow: "hidden" }}>
          <div style={{ display: "flex", gap: 80, alignItems: "center", animation: "lpgMq 28s linear infinite", willChange: "transform", flexShrink: 0 }}>
            {allLogos.map((logo, i) => (
              <img key={i} src={logo.src} alt={i < logos.length ? logo.name : ""} aria-hidden={i < logos.length ? undefined : true}
                style={{ height: 26, objectFit: "contain", filter: "grayscale(1) brightness(0)", opacity: 0.35, flexShrink: 0, maxWidth: 110 }} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="servicios" style={{ backgroundColor: "#FFFFFF", position: "relative", overflow: "hidden", borderTop: "1px solid rgba(0,0,0,0.06)" }}>
        <div className="lpg-pad" style={{ maxWidth: 1440, margin: "0 auto", paddingLeft: 48, paddingRight: 48, paddingTop: 100, paddingBottom: 100, position: "relative" }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 64 }}>
            ...
            <span style={{ fontFamily: FONT, fontWeight: 400, fontSize: 13, color: DIM_LIGHT, alignSelf: "flex-end", paddingBottom: 8, letterSpacing: "0.06em" }}>04 áreas</span>
          </div>
          {SVCS.map((s, i) => (
            <Link key={s.num} to={s.href} style={{ textDecoration: "none", display: "block" }}
              ...
              <div style={{ borderTop: `1px solid ${svcHov === i ? B3 + "44" : "rgba(0,0,0,0.08)"}`, padding: "32px 0", cursor: "pointer", transition: "background 0.28s ease", background: svcHov === i ? `linear-gradient(90deg, ${B3}08 0%, transparent 70%)` : "transparent", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 24, position: "relative" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 24, flex: 1, minWidth: 0 }}>
                  <span className="hidden sm:block" style={{ fontFamily: FONT, fontWeight: 400, fontSize: 12, letterSpacing: "0.12em", color: svcHov === i ? B3 + "99" : DIM_LIGHT, flexShrink: 0, transition: "color 0.28s" }}>{s.num}</span>
                  ...
                  <div className="hidden lg:flex" style={{ gap: 8, flexWrap: "wrap", flex: 1 }}>
                    {s.tags.map(tag => (<span key={tag} style={{ fontFamily: FONT, fontWeight: 400, fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase", color: svcHov === i ? B3 + "CC" : DIM_LIGHT, border: `1px solid ${svcHov === i ? B3 + "33" : "rgba(0,0,0,0.1)"}`, padding: "4px 10px", transition: "color 0.28s, border-color 0.28s" }}>{tag}</span>))}
                  </div>
                </div>
                <svg width="28" height="28" viewBox="0 0 28 28" fill="none" style={{ flexShrink: 0, transform: svcHov === i ? "rotate(45deg)" : "rotate(0deg)", transition: "transform 0.28s ease" }}>
                  <path d="M6 22L22 6M22 6H8M22 6V20" stroke={svcHov === i ? B3 : FAINT_LIGHT} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
        ...
        <div className="lpg-pad grid grid-cols-2 md:grid-cols-4" style={{ maxWidth: 1440, margin: "0 auto", paddingLeft: 48, paddingRight: 48 }}>
          {[
            ...
            <div key={d.name} className="lpg-div-item" style={{ padding: "64px 0", paddingRight: 40, borderRight: i < 3 ? `1px solid ${W08}` : "none", paddingLeft: i > 0 ? 40 : 0 }}>
              ...
              {d.items.map(item => (<div key={item} style={{ fontFamily: FONT, fontWeight: 300, fontSize: 13, color: DIM_DARK, marginBottom: 8, lineHeight: 1.6 }}>{item}</div>))}
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ backgroundColor: "#FFFFFF", position: "relative", overflow: "hidden" }}>
        <div className="lpg-pad" style={{ maxWidth: 1440, margin: "0 auto", paddingLeft: 48, paddingRight: 48, paddingTop: 100, paddingBottom: 100, position: "relative" }}>
          <h2 style={{ fontFamily: FONT, fontWeight: 900, fontSize: "clamp(60px, 8vw, 100px)", letterSpacing: "-0.04em", lineHeight: 1, color: INK, userSelect: "none", marginBottom: 48 }}>NOSOTROS</h2>
          ...
          <div className="grid grid-cols-1 gap-10 items-start md:grid-cols-2 md:gap-[100px]">
            ...
            <div>
              ...
              <p style={{ fontFamily: FONT, fontWeight: 300, fontSize: 17, lineHeight: 1.8, color: BODY_LIGHT, marginBottom: 48 }}>
              ...
              <div style={{ borderTop: "1px solid rgba(0,0,0,0.08)", paddingTop: 48, display: "grid", gridTemplateColumns: "1fr 1fr", rowGap: 40, columnGap: 24 }}>
                {[
                  ...
                  <div key={l}>
                    <div style={{ fontFamily: FONT, fontWeight: 900, fontSize: 48, letterSpacing: "-0.04em", color: INK, lineHeight: 1 }}>{n}</div>
                    <div style={{ fontFamily: FONT, fontWeight: 400, fontSize: 13, color: DIM_LIGHT, marginTop: 8 }}>{l}</div>
                  </div>
                ))}
              </div>
        ...
        <div className="lpg-pad" style={{ maxWidth: 1440, margin: "0 auto", paddingLeft: 48, paddingRight: 48, paddingTop: 100, paddingBottom: 100, position: "relative" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: FONT, fontWeight: 300, fontSize: 12, letterSpacing: "0.06em", color: DIM_DARK, marginTop: 4 }}>CEO · Constructora Nacional</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ backgroundColor: "#FFFFFF", position: "relative", overflow: "hidden" }}>
        <div className="lpg-pad" style={{ maxWidth: 1440, margin: "0 auto", paddingLeft: 48, paddingRight: 48, paddingTop: 100, paddingBottom: 100, position: "relative" }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", borderBottom: `1px solid #00000014`, paddingBottom: 40, marginBottom: 56 }}>
            <h2 style={{ fontFamily: FONT, fontWeight: 900, fontSize: "clamp(40px, 5.5vw, 80px)", letterSpacing: "-0.05em", lineHeight: 0.85, color: INK }}>INSIGHTS</h2>
            ...
            {INS.map((item, i) => (
              <div key={i} onMouseEnter={() => setInsHov(i)} onMouseLeave={() => setInsHov(null)} style={{ cursor: "pointer", display: "flex", flexDirection: "column", height: "100%" }}>
                ...
                <div style={{ padding: "24px 0", borderBottom: `1px solid #00000014`, display: "flex", flexDirection: "column", flex: 1 }}>
                  <div style={{ fontFamily: FONT, fontWeight: 600, fontSize: 10, letterSpacing: "0.22em", textTransform: "uppercase", color: GRAY, marginBottom: 10 }}>{item.cat}</div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <span style={{ fontFamily: FONT, fontWeight: 300, fontSize: 12, color: DIM_LIGHT }}>{item.date}</span>
                    <span style={{ fontFamily: FONT, fontWeight: 500, fontSize: 12, letterSpacing: "0.08em", textTransform: "uppercase", color: insHov === i ? B3 : DIM_LIGHT, transition: "color 0.2s" }}>Leer →</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 56, flexWrap: "wrap", gap: 16 }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                <div style={{ width: 5, height: 5, borderRadius: "50%", backgroundColor: GREEN }} />
                <span style={{ fontFamily: FONT, fontWeight: 400, fontSize: 11, letterSpacing: "0.22em", textTransform: "uppercase", color: DIM_DARK }}>Presencia en Colombia</span>
              </div>
            ...
            <div style={{ alignSelf: "flex-end", paddingBottom: 8 }}>
              ...
              <div style={{ fontFamily: FONT, fontWeight: 300, fontSize: 12, letterSpacing: "0.12em", textTransform: "uppercase", color: DIM_DARK, marginTop: 4 }}>Bogotá · Cali · Medellín</div>
            </div>
          </div>
                ...
                <div style={{ borderTop: `1px solid ${W08}`, paddingTop: 20, display: "flex", flexDirection: "column", gap: 14 }}>
                  {[
                    ...
                    <div key={row.label}>
                      ...
                      <div style={{ fontFamily: FONT, fontWeight: 300, fontSize: 12, color: DIM_DARK, lineHeight: 1.5 }}>{row.value}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* ── CTA / Hablemos ── */}
      <section style={{ backgroundColor: B0, position: "relative", overflow: "hidden" }}>
        <div className="lpg-pad" style={{ maxWidth: 1440, margin: "0 auto", paddingLeft: 48, paddingRight: 48, paddingTop: 100, paddingBottom: 100, position: "relative" }}>
          {/* Removed inline display/gridTemplateColumns — Tailwind responsive classes handle layout */}
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-[80px]" style={{ alignItems: "start" }}>
            ...
            <div>
              ...
              {[
                ...
                <div key={label} style={{ borderTop: `2px solid ${B3}`, paddingTop: 20, paddingBottom: 21 }}>
                  <div style={{ fontFamily: FONT, fontWeight: 600, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: DIM_DARK, marginBottom: 6 }}>{label}</div>
            ...
            <div>
              {[
                ...
                <div key={label} style={{ marginBottom: 32 }}>
                  <label style={{ fontFamily: FONT, fontWeight: 600, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: DIM_DARK, display: "block", marginBottom: 12 }}>{label}</label>
                  <input
                    type={type}
              ...
              <div style={{ marginBottom: 48 }}>
                <label style={{ fontFamily: FONT, fontWeight: 600, fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: DIM_DARK, display: "block", marginBottom: 12 }}>Mensaje</label>
                <textarea
          ...
          <style>{`.lpg-cta-input::placeholder { color: rgba(255,255,255,0.62); } .lpg-cta-input:focus { border-bottom-color: ${GREEN}; border-bottom-width: 2px; padding-bottom: 13px; outline: 2px solid transparent; }`}</style>
        </div>
      </section>
    </>
  );
